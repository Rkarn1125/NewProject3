...
// Assets dir (not used in Cloud Functions for static serving)
const assetsDir = path.join(os.tmpdir(), 'assets');
...
async function handleOpenRouterEnrichment(req, res) {
  try {
    ...
    const response = await fetchWithTimeout("https://openrouter.ai/api/v1/chat/completions", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${globalThis.__OPENROUTER_API_KEY}`,
        "HTTP-Referer": "https://faceup-39235.web.app/",
...
// Use /tmp for writable directories in Cloud Functions
const profilesDataDir = path.join(os.tmpdir(), 'data', 'profiles');
const avatarsDir = path.join(os.tmpdir(), 'assets', 'avatars');
try {
...
export const api = onRequest(
  ...
  (req, res) => {
    // Make the secret available for handler functions
    const OPENROUTER_API_KEY = openrouterApiKey.value();
    globalThis.__OPENROUTER_API_KEY = OPENROUTER_API_KEY;
    ...
    // All other routes return 404 (static files served by Firebase Hosting)
